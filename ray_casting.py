import pygame
import settings
from map import world_map, door_map
...
def ray_casting(sc, player_pos, player_angle, textures):
    ...
    for ray in range(settings.NUM_RAYS):
        sin_a = settings.math.sin(cur_angle)
        cos_a = settings.math.cos(cur_angle)
        sin_a = sin_a if sin_a else 0.000001
        cos_a = cos_a if cos_a else 0.000001

        # verticals
        x, dx = (xm + settings.TILE, 1) if cos_a >= 0 else (xm, -1)
        for i in range(0, settings.WIDTH, settings.TILE):
            depth_v = (x - ox) / cos_a
            yv = oy + depth_v * sin_a
            ...
            if tile_v in world_map:
                ...
                break
            if tile_v in door_map:
                texture_v = door_map[tile_v]
                break
            x += dx * settings.TILE

        # horizontals
        y, dy = (ym + settings.TILE, 1) if sin_a >= 0 else (ym, -1)
        for i in range(0, settings.HEIGHT, settings.TILE):
            depth_h = (y - oy) / sin_a
            xh = ox + depth_h * cos_a
            ...
            if tile_h in world_map:
                ...
                break
            if tile_h in door_map:
                texture_h = door_map[tile_h]
                break
            y += dy * settings.TILE
        ...
        depth *= settings.math.cos(player_angle - cur_angle)
        ...
        sc.blit(wall_column, (ray * settings.SCALE, settings.HALF_HEIGHT - proj_height // 2))
